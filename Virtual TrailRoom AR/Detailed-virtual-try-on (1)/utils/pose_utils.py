...
def pose2loc(pose):
    ...
    for i,j in zip(x, y):
        ...
        if j > 255:
            j = 255
        loc.append([j, i])
    print(len(loc))# attention the indices 
    return loc[0:18]
...
def draw_pose_from_cords(pose_joints, img_size, radius=6, draw_joints=True):
    ...
    if draw_joints:
        for f, t in LIMB_SEQ:
            ...
            mask[yy, xx] = True
#FIE21D00C-A11@9=person_whole_back.jpg	4BE21D0A2-C11@8=person_half_front_keypoints.json	7SI21D04N-G11@10=cloth_front.jpg	test
#fashion_new.jpg	fashion_new_keypoints.json	7SI21D04N-G11@10=cloth_front.jpg	test  
            #print(len(pose_joints))
